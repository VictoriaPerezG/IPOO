...
class ViajesTerrestres extends Viaje{
    ...
    public function venderPasaje($pasajero)
    {
        ...
        $importe = parent::venderPasaje($pasajero);//si es semicama devuelve este importe
        
        if ($importe <> false)//si lo que devuelve la variable $importe es distinto que false entra a la alternativa
        {
            if($comodidadAsiento == "cama"){
                $porcentaje = (( 25 * $importe) / 100 ); 
                $importe = $importe + $porcentaje;//si es cama se devuelve este importe
            }
